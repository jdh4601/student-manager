...
**Acceptance Criteria:**
- [ ] 과목별 점수(0~100) 입력 후 총점·평균이 즉시 표시된다
- [ ] 한국 교육과정 9등급제 기준으로 등급(1~9)이 자동 산출된다 (석차 기반이 아닌 원점수 기준 설정 가능)
- [ ] 성적 수정 시 계산값이 실시간으로 재반영된다
- [ ] 입력 범위 초과(음수, 100 초과) 시 즉시 오류 메시지가 표시된다
- [ ] 저장 버튼 클릭 없이 자동 저장(AutoSave)되거나 명시적 저장 버튼이 존재한다
...
**Acceptance Criteria:**
- [ ] 레이더 차트에 등록된 모든 과목이 축으로 표시된다
- [ ] 여러 학기 데이터를 겹쳐 보는 비교 모드가 제공된다
- [ ] 차트를 PNG/PDF로 내보낼 수 있다
- [ ] 학생이 본인 성적 조회 시에도 동일한 차트를 볼 수 있다
...
**Acceptance Criteria:**
- [ ] 상담 날짜·주요 내용·다음 상담 계획을 필드별로 입력할 수 있다
- [ ] 공유 여부를 상담 건별로 설정할 수 있다 (기본값: 공유됨)
- [ ] 같은 학교 교사는 공유된 상담 내역을 학생명·기간·작성 교사로 검색·필터링할 수 있다
- [ ] 상담 내역은 학생·학부모에게 노출되지 않는다
- [ ] 상담 기록은 작성자만 수정 가능, 공유 시 다른 교사는 읽기만 가능
...
**Acceptance Criteria:**
- [ ] 피드백 카테고리(성적/행동/출결/태도)를 선택할 수 있다
- [ ] 학생 공개·학부모 공개·비공개를 독립적으로 설정할 수 있다
- [ ] 공개된 피드백은 학생·학부모 로그인 후 즉시 확인 가능하다
- [ ] 피드백 작성 시 관련 학생·학부모에게 알림이 발송된다
- [ ] 교사는 이미 공개된 피드백을 수정하거나 비공개로 되돌릴 수 있다
...
**Acceptance Criteria:**
- [ ] 로그인 후 본인(자녀) 데이터만 조회된다 (타 학생 데이터 접근 불가)
...
**Acceptance Criteria:**
- [ ] 학생 기본 정보(이름·학년·반·번호·생년월일) CRUD가 가능하다
- [ ] 출결 상태(출석/결석/지각/조퇴)를 날짜별로 입력할 수 있다
- [ ] 특기사항을 자유 텍스트로 입력할 수 있다
- [ ] 학생 정보를 CSV로 일괄 가져오기·내보내기 할 수 있다
...
**Acceptance Criteria:**
- [ ] 알림은 인앱(In-App) 방식으로 제공된다 (v1 기준; 이메일은 v2)
- [ ] 알림 목록에서 읽음/안 읽음 상태를 관리할 수 있다
- [ ] 알림 클릭 시 해당 학생의 관련 화면으로 이동한다
- [ ] 알림 설정에서 알림 유형별 ON/OFF가 가능하다
...
| REQ-004 | 교사가 학생·학부모 계정을 직접 생성 | Must | S1 |
...
| Sprint | 주차 | 담당 REQ | 누적 Must 완료 |
|--------|------|---------|--------------|
| S0 | 1–2주 | 설계 산출물 (REQ 없음) | 0% |
| S1 | 3–4주 | REQ-001~004, 010, 011, 012(proto) | 27% |
| S2 | 5–6주 ⭐ | REQ-012(완성), 015, 020~022 | 55% |
| S3 | 7–8주 | REQ-030~032 | 68% |
| S4 | 9–10주 | REQ-033, 040~042, 050, 051 | 91% |
| S5 | 11–12주 ⭐ | REQ-014, 023, 060, QA, 버퍼 | 100% |

> 버퍼 항목 (S5): REQ-005, 013, 043, 061, 062 — 시간 여유 시 구현, 미완 시 v2로 이동
...
```python
# 9등급제 (원점수 기준 — 석차 등급이 아닌 절대평가 대안으로 구현)
# 실제 한국 고교 석차등급은 석차백분율 기반이나,
# MVP에서는 원점수 기준 참고 등급 제공 (추후 석차 기반으로 전환 가능)
GRADE_CUTOFFS = [96, 89, 77, 60, 40, 23, 11, 4]  # 상위 누적 비율 기준 점수 경계

def calculate_grade(score: float) -> int:
    # 별도 구현 — 클래스 내 석차 데이터 없을 경우 원점수 참고값 반환
    ...
...
**Story Points**: ~10pt | **Review**: 로그인 → 학생 선택 → 성적 입력 데모
...
**중간 발표 데모**: 교사 로그인 → 성적 입력 → 레이더 차트 → 학생부 조회 → 학생 계정으로 성적 조회
...
**최종 발표 데모**: 교사 로그인 → 성적 입력·레이더 차트 → 피드백 작성·학부모 공개·학부모 확인 → 상담 기록·공유 → PDF/Excel 다운로드
...
- **학년**: 중학교 1~3학년, 고등학교 1~3학년
- **성적 등급**: 고교 9등급제 (석차백분율 기반); MVP에서는 원점수 참고 등급 제공
- **학생부**: 학교생활기록부 — 성적 외 출결·특기사항·봉사활동 등 포함
- **NEIS**: 교육부 나이스 — 본 시스템과 직접 연동 없음 (CSV 가져오기·내보내기만)

### C. 원본 참조 문서

- `고객요구사항명세서.md` — 초기 고객 요구사항 명세서 v1.0 (2025-03-14)
- `plan.md` — 12주 애자일 개발 계획 + 기술 스택 결정 (2026-03-19)
- PRD v1.0: 6라운드 인터뷰 기반 (`docs/PRD.md` v1)

### D. 인터뷰 요약 (PRD v1 기반)

| 라운드 | 질문 | 결정 |
|--------|------|------|
| 1 | NEIS 연동 여부 | CSV/Excel 선택적 연동 |
| 2 | 배포 방식 | SaaS 클라우드 |
| 3 | 성공 기준 | 핵심 기능 오류 없이 동작 |
| 4 | 초기 규모 | 1~10학교 (MVP) |
